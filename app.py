...
with st.sidebar:
    ...
    with st.expander("⚙️ 高级配置 (班级规则与姓名映射)", expanded=False):
        tab_rule, tab_map, tab_tpl = st.tabs(["🎯 规则配置", "🔤 姓名映射", "📝 模板编辑"])

        with tab_rule:
            class_input = st.text_input("班级名称 (如: 康乐E4)")
            if class_input:
                t_listen = st.number_input("听音(分钟)", value=st.session_state.class_rules.get(class_input, {}).get("listen", 20))
                t_anim = st.number_input("动画(分钟)", value=st.session_state.class_rules.get(class_input, {}).get("anim", 10))
                t_books = st.number_input("绘本(本)", value=st.session_state.class_rules.get(class_input, {}).get("books", 1))
                if st.button("保存班级规则"):
                    st.session_state.class_rules[class_input] = {"listen": t_listen, "anim": t_anim, "books": t_books}
                    sync_to_db()
                    st.success(f"已保存 {class_input} 规则！")

        with tab_map:
            map_class_input = st.text_input("对应班级 (如: 康乐E4)", key="map_class")
            map_text_input = st.text_area("映射列表 (中文:英文)", value=st.session_state.name_maps.get(map_class_input, ""), placeholder="林乐铠:Carl\n吴斯涵:Tian", height=100)
            if st.button("保存姓名映射"):
                if map_class_input:
                    st.session_state.name_maps[map_class_input] = map_text_input
                    sync_to_db()
                    st.success(f"已保存 {map_class_input} 映射！")

        with tab_tpl:
            if mode_key == "matrix":
                current_tpl = st.text_area("矩阵模板", value=st.session_state.matrix_template, height=140)
                if current_tpl != st.session_state.matrix_template:
                    st.session_state.matrix_template = current_tpl
                    sync_to_db()
            else:
                current_tpl = st.text_area("传统模板", value=st.session_state.traditional_template, height=140)
...
if btn_generate:
    if not st.session_state.auth_token:
        st.error("请先在左侧边栏登录或填入有效的 Token！")
    else:
    ...
    st.info("👈 请在左侧边栏选择模式与时间，点击 **「🚀 立即生成学情报告」** 即可。")
